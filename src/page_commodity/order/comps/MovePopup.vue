<style lang="scss">
  ...
  .close-bottom-text {
    height: 140rpx;
    border-top: 20rpx #f5f6fa solid;
    display: flex;
    justify-content: center;
    align-items: center;
    color: rgba(51, 50, 40, 1);
    font-size: 32rpx;
    font-weight: 400;
  }
</style>
...
<template>
  <uni-popup ref="popup" :mask-click="maskClick" :type="from" class="hqs-popup" @change="onChange">
    <view
      class="qs-con"
      :style="conStyle"
      @mousedown="onTouch"
    ...
    >
      <block v-if="isVertical">
        <view v-if="from == 'bottom' && showHeader" class="qs-header">
          ...
          <view class="qs-side ta-r" :class="{ hidden: !showClose }" @click="close">
            <slot name="close">
              <u-icon v-if="closeIcon" :name="closeIcon" :size="32" />
              <text v-else> 关闭 </text>
            </slot>
          </view>
        </view>
        ...
        <slot name="bottom">
          <p v-if="bottomClose" class="close-bottom-text" @click="close">取消</p></slot
        >
      </block>
...
<script>
  import UniPopup from '@/components/MovePopup/uni-popup.vue';
