  ...
  <view class="chapter-index-wrap" :style="[config.style]">
    ...
    <view :style="{ display: noData ? 'none' : 'block' }">
      ...
      <div class="recursion-container">
        <RecursionLevelList :list="courseList" :showSpeed="true" :commodityId="commodityId" :supplierId="supplierId" />
      </div>
    </view>
...
<script>
  ...
  import { flatList, getProductIds, assembleData } from '@/course/chapter/utils/index';
  export default {
    name: 'ChapterIndex',
    components: {
      LearnInfo,
    ...
    methods: {
      ...
      async getCourseInfo(first) {
        ...
        this.commodityId = result.body.commodityId;
        this.supplierId = result.body.supplierId;
        if (first) {
          const tabs = result.body.types;
          tabs.map((item) => {
            switch (item) {
              case 0:
          ...
          let constructs = result?.body?.typeProductTrees[firstTab] || [];
          // 利用缓存重新组织树结构
          ...
          // this.courseInfo = result.body;
          let constructs = result?.body?.productTrees || [];
          // 利用缓存重新组织树结构
          const productIds = getProductIds(constructs, 'P');
          const statics = await fetchGetStudyStat({ productIds });
          constructs = assembleData(constructs, statics.body);
