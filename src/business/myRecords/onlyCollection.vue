...
<template>
  <view class="wrap-question flex flex-column">
    ...
    <!-- #endif -->
    <!-- #ifdef MP-BAIDU -->
    <view class="bottom-box between-box" v-if="activeNum == 1">
      <view class="flex-column-center" @tap="toReturn()">
        <!-- <view class="t-icon t-icon-aixin image-style iconfont exit-icon" /> -->
        <view class="t-icon image-style icon-power iconfont exit-icon" />
...
<script>
  ...
  // import ReturnModal from '../brushQuestions/comps/ReturnModal.vue';
  export default {
    name: 'wrongOrCollection',
    components: { QuestionSwiper, QuestionComp, ReturnModal, ReturnModal },
    onLoad(options) {
